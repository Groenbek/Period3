 ...
 */
 router.put('/editme/', async function (req: any, res, next) {
  try {
    ...
    const changed = await facade.editFriend(email, newFriend)
    res.json(changed)
  } catch (err) {
    debug(err)
    if (err instanceof ApiError) {
...
router.get("/find-user/:email", async (req: any, res, next) => {
  try {
      ...
      const friend = await facade.getFriend(uEmail);
      if (friend == null) {
...
router.put('/:email', async function (req: any, res, next) {
  ...
  try {
      ...
      const uEmail = req.params.email;
      let newFriend = req.body;
      const result = await facade.editFriend(uEmail, newFriend);
      res.json(result);
...
router.delete('/delete/:email', async function (req: any, res, next) {
  ...
  try {
      ...
      const uEmail = req.params.email;
      const result = await facade.deleteFriend(uEmail);
      res.json(result);
